...
SupportPercent <- function(plottedTree) {
  round(100 * SplitFrequency(plottedTree, trees) / length(trees))
}

plot(plenary,
     tip.color = ifelse(plenary$tip.label %in% rogueTaxa, 2, 1))
LabelSplits(plenary, SupportPercent(plenary), unit = "%")
plot(reduced)
LabelSplits(reduced, SupportPercent(reduced), unit = "%")
```
