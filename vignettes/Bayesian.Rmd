...
The code below overwrites `trees` with a thinned sample of `sampleSize` trees;
the results that follow are for this thinned sample, and analysing a different
sample (such as every post-burn-in tree) will give different results.
...
`QuickRogue()` is a heuristic, and the taxa it identifies depend on the trees
in the sample; a different sample may reveal a different set of rogues.
...
`SplitFrequency()` counts the trees that contain each split; we divide by the
number of trees to label each split with its percentage support.
...
LabelSplits(plenary, round(100 * SplitFrequency(plenary, trees) / length(trees)),
            unit = "%")
plot(reduced)
LabelSplits(reduced, round(100 * SplitFrequency(reduced, trees) / length(trees)),
            unit = "%")
```
