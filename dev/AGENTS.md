...
taxa in phylogenetic tree sets. Rogue taxa have uncertain positions that
reduce consensus tree resolution; removing them can increase information
...
nodes. VTune profiling shows the inner-loop `SETBOTH` macro (~line 56)
accounts for ~62% of Rogue.dll time — this is fundamental to the
algorithm and not easily improvable without a redesign.

## Dependencies

**Key imports**: ape (phylo I/O), TreeTools (tree manipulation),
TreeDist (information measures), Rfast (fast row stats: `rowMads`,
`rowMedians`, `rowVars`, `rowmeans`), matrixStats (supplementary),
fastmatch (fast matching), cli (progress bars).
...
- **Auto-parallel heuristic**:
  [`TipInstability()`](https://ms609.github.io/Rogue/dev/reference/TipInstability.md)
  automatically enables OpenMP in Rfast operations when
  `nrow(dists_lt) > 1000L`, avoiding overhead on small problems.
